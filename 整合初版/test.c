...
typedef struct _DATA_SOURCE{
	...
	int state;			  // I\Oå£çŠ¶æ€ï¼Œé«˜1ï¼Œä½Ž0
	unsigned char H8;
	unsigned char L8;
}DATA_SOURCE;
...
/** ¶ÁÈ¡Ä£ÄâÁ¿Êý¾Ý£¬²¢½«¸ß°ËÎ»ºÍµÍ°ËÎ»·Ö¿ª·ÅÔÚÊý×éÖÐ **/
...
/** ¶ÁÈ¡Ä£ÄâÁ¿³õÖµ **/
void Modbus_read_AO()
	{   modbus_read_registers(ctx,analog,4,tab_ana);
		int i;
		 for(i=0;i<=3;i++)
	   {
		 LoadOutputAnalog(i);
...
int main()
{
		...
		ret1=pthread_create(&opcua_server_id,NULL,(void *)handle_opcua_server,NULL);
		if(ret!=0||ret1!=0)
        {
